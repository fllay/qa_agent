...
- Compressed the large-graph node-size range so the biggest repository and high-degree nodes stop dwarfing the rest of the graph: lowered the repository min/max/growth curve while slightly lifting the small-node floor, keeping dense graphs clickable without the extreme size contrast; bumped the frontend asset version to `20260618-graph-light13`.
- Lowered the dense large-graph node-size range again and spread the zoomed-out overview wider by raising large-graph repulsion, link distance, and simulation space while reducing fit-view padding, so the full Graphify graph uses more of the canvas without the same dense central blob; bumped the frontend asset version to `20260618-graph-light14`.
- Lowered the dense large-graph node-size range further and spread the zoomed-out overview wider again by tightening the repository/high-degree growth curve, increasing large-graph repulsion and link distance further, expanding large/huge simulation space again, and trimming fit-view padding down another step; bumped the frontend asset version to `20260618-graph-light15`.
- Made the node-size range global by replacing the split small/large graph radius logic with one shared family-aware radius curve plus a smooth graph-size multiplier, so all graph sizes now follow the same sizing behavior instead of diverging sharply once the graph crosses the large-graph threshold; bumped the frontend asset version to `20260618-graph-light16`.
- Raised the new global node-size curve again after the first shared pass came out too small, by increasing the shared non-large radius multiplier plus the family-aware floors, caps, and base growth values, and by lifting the small/medium/default Cosmograph point-size bases to bring fallback and medium graphs closer to the earlier larger feel; bumped the frontend asset version to `20260618-graph-light17`.
- Raised the global node-size curve again across all graph sizes by lifting both branches of the shared graph-model radius multiplier plus the family-aware floors, caps, and base growth values, and by increasing every Cosmograph point-size base band so small, medium, large, and huge graphs all render larger together; bumped the frontend asset version to `20260618-graph-light18`.
- Flattened the remaining graph-size-dependent node scaling so Graphify and fallback graphs share the same node-size range: Cosmograph now uses one global `pointSizeBase`, and the graph model uses a constant `radiusScale` instead of separate large/non-large multipliers; bumped the frontend asset version to `20260618-graph-light19`.
